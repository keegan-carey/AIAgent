...
PM_PERSONA = Persona(
    ...
    system_prompt=(
        ...
        "For each page in your plan, set `skill_id` to one of the available skills "
        "when it fits the page type. The most relevant skills for the current brief "
        "will be listed in the user message under `Available skills` — pick from that "
        "ranked list (top entries are most relevant). Set `skill_id` to `null` (omit "
        "it) when no listed skill clearly fits.\n\n"
        "Think about:\n"
...
PROJECT_DEVELOPER_PERSONA = Persona(
    ...
    system_prompt=(
        ...
        "COMPONENT LIBRARY:\n"
        "Builtin blocks and components saved in this project's library are "
        "listed in your brief — call list_blocks() / list_project_components() "
        "for the current catalog. When a planned section matches one, PREFER "
        "render_block(id_or_slug, config) and adapt the rendered HTML instead "
        "of writing the markup from scratch.\n\n"
        "EDITING RULES:\n"
...
PROJECT_REVIEWER_PERSONA = Persona(
    ...
    system_prompt=(
        ...
        "When screenshots of the rendered site are attached, they are your "
        "PRIMARY evidence: judge layout, visual hierarchy, spacing, typography, "
        "responsiveness (mobile shots), and overall polish from what the user "
        "will actually see. Read code only to diagnose problems you observe. "
        "An automated verification report may also be provided — its console "
        "errors and failed requests are facts, not opinions.\n\n"
        "Review process:\n"
...
SECTION_PERSONA = Persona(
    ...
    system_prompt=(
        ...
        "- When the brief lists reusable components and your section matches "
        "one, call render_block(slug, config) and adapt the rendered markup "
